...
class SSD1306:
    def __init__(self, width, height, external_vcc):
        ...
        self.poweron()
        self.init_display()
        self.__fontlib = FontLib('/combined.bin')#加载字体文件
    ...
    def hz(self, chars, x=0, y=0):
        buffer_dict = self.__fontlib.get_characters(chars)  # 取全部字模
        ...
        for char in chars:
            ...
            if x > ((self.width // width - 1) * width):
                x = 0
                y += height
            if y > ((self.height // height - 1) * height):  # 换行
                self.show()
                self.fill(0)
                time.sleep(3)
                x = y = 0
            buffer = framebuf.FrameBuffer(bytearray(
                buffer), self.__fontlib.font_width, self.__fontlib.font_height, self.__fontlib.format)
            self.blit(buffer, x, y)
            if ord(char) < 127:  # 判断是ascii字符
                x += width // 2
            else:
                x += width
...
class SSD1306_SPI(SSD1306):
    ...
    def write_cmd(self, cmd):
        ...
        if currentBoard == "esp8266" or currentBoard == "esp32":
            ...
            self.spi.init(mode=pyb.SPI.MASTER,
                          baudrate=self.rate, polarity=0, phase=0)
        self.cs.high()
        self.dc.low()
        self.cs.low()
        global currentBoard
